...
<form class="pkp_form" id="repecSettingsForm" method="post" enctype="multipart/form-data" action="{url router=\PKP\core\PKPApplication::ROUTE_COMPONENT op="manage" category="generic" plugin=$pluginName verb="settings" save=true}" data-article-handle-pattern-locked="{if $articleHandlePatternLocked}1{else}0{/if}" data-article-handle-pattern-preview-archive-code="{$articleHandlePatternPreviewArchiveCode|escape}" data-article-handle-pattern-preview-series-code="{$articleHandlePatternPreviewSeriesCode|escape}">
	{csrf}
